    ...
    switch (Sub) {
      ...
      case "transcript": {
        ...
        if (interaction.channel.name.startsWith(`ticket-`) || interaction.channel.name === ticketName) {
          ...
          const trans = await require(`${process.cwd()}/functions/transcript`)(interaction)
          if(!trans) return interaction.reply({
            content: "Got an error while creating transcript"
          })
          ...
          await interaction.user.send({
             components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setURL(`${trans}`).setLabel("Transcript").setStyle(ButtonStyle.Link))],
            embeds: [new EmbedBuilder().setColor(client.colors.none).setDescription(`${client.emotes.success}| Successfully Transcript Created`)],
          })
        } else {
          errorMessage(client, interaction, `**My Friend, here is not a ticket channel please use this command in other channel**`)
        }
