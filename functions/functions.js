...
module.exports = {
  logMessage: async function(client, interaction, channel, message, reason, emote, has_file, file) {
    ...
    if (has_file === true) {
      return channel.send({
        components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setURL(`${file}`).setLabel("Transcript").setStyle(ButtonStyle.Link))],
  ...
  errorMessage: async function(client, interaction, error) {
    ...
    return interaction.reply({
      embeds: [new EmbedBuilder().setTitle('⛔️| **We Got An Error**').setColor(client.colors.red).setDescription(`${error}`).setFooter({ text: `Requested by ${member.tag} • Error • ${client.embed.footerText}`, iconURL: member.displayAvatarURL({ dynamic: true }) }).setThumbnail(interaction.guild ? interaction.guild.iconURL({ dynamic: true }) : "")],
